...
#                     user_agent="najnesnaj@gmail.com", count=4)
    ...
    def insertdb(self, data):
        ...
        for i, stock in enumerate(data.index):
            #print ("i",i)
            aantal = float(data['shares_current'].iloc[i])
            waarde =  float(data['value_current'].iloc[i]*1000)
            if (data['value_current'].iloc[i] > data['value_previous'].iloc[i]):
                sellorbuy = "buy" 
            else:
                sellorbuy = "sell" 
            if aantal != 0 :
                ...
                prijs = 0
            aantalgekocht = aantal - float(data['shares_previous'].iloc[i])
            bedrag = waarde - float(data['value_previous'].iloc[i]*1000)
            #45 day delay in reporting
            #men rapporteert over een kwartaal = (3 maand) 45 is daar de helft van
            vermoedelijkedatum=str(self.current_report_date  - timedelta(days = 45))
            #print (str(stock), " price: ", prijs)
            transrecord = []
            transrecord.append(self.Name + str(stock) + str(self.current_report_date))
            transrecord.append(str(stock))
            transrecord.append(bedrag)
            transrecord.append(sellorbuy)
            #transrecord.append(str(self.current_report_date))
            transrecord.append(vermoedelijkedatum)
            transrecord.append(str(self.current_filing_date))
            #F stands for funds
            transrecord.append("F")
            #transrecord.append(data['cusip'].iloc[i])
            transrecord.append("US")
            #this field can be used for something else
            transrecord.append("DLR")
            transrecord.append(aantalgekocht)
            transrecord.append(prijs)
            transrecord.append(self.Name)
            ...
            try:
                ...
                "howmany"       ,
                "shareprice"    ,
                "extrainfo"     ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)''' , (transrecord[0:12]))
                conn.commit()
                #print (transrecord)
    ...
    def compare_recent_changes(self):
        ...
        #print(data_current)
        #print("shares_current")
        #print (data_current['shares_current'])
        #data_current =  (data_current.groupby(level=[0]).sum())
        #within the filings there are double entries
        data_current =  (data_current.groupby(level=[0]).sum())
        data_previous =  (data_previous.groupby(level=[0]).sum())
    ...
    def plot_recent_shares_change(self, data):
        ...
        # Initialize plot
        SIZE = 18
        SIZEY= int(len(data.index) / 5)
        XLIM = 125
        YLIM = -1
        print ("aantal", len(data.index)) 
    ...
    def plot_recent_value_change(self, data):
        ...
        ax.set_xlabel('Percentage',fontweight="bold")
        plt.savefig(self.Name + 'fractional' + str(self.current_report_date))
        #plt.show() 
        return
